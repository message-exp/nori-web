 ...
 */
export async function getBaseUrl(userId: string): Promise<string | "IGNORE" | "FAIL_PROMPT" | "FAIL_ERROR"> {
  ...
  try {
    response = await fetch(baseUrl.replace(/\/+$/, "") + "/_matrix/client/versions")
    if (!response.ok) {
      throw new Error(`${response.status}`);
    }
  } catch (error) {
  ...
  if (data?.["m.identity_server"]?.base_url) {
    ...
    try {
      response = await fetch(identityBaseUrl.replace(/\/+$/, "") + "/_matrix/identity/v2")
      if (!response.ok) {
        throw new Error(`${response.status}`);
      }
    } catch (error) {
